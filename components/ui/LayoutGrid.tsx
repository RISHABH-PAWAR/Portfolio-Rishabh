...
export const LayoutGrid = ({ cards }: { cards: Card[] }) => {
  ...
  return (
    ...
    <div className="w-full min-h-fit p-10 grid grid-cols-1 md:grid-cols-4 max-w-7xl mx-auto gap-10 ">
      {cards.map((card, i) => (
        <Button
          key={i}
          ...
          >
            <motion.div
              ...
              className={cn(
                ...
                selected?.id === card.id
                  ? "rounded-lg cursor-pointer fixed inset-0 mx-auto my-auto h-1/2 w-full md:w-1/2 m-auto z-50 flex justify-center items-center flex-wrap flex-col"
                  : lastSelected?.id === card.id
                  ? "z-40 bg-white rounded-xl h-full w-full"
                  : "bg-white rounded-xl h-full w-full"
              )}
              layout={selected?.id !== card.id}
            >
